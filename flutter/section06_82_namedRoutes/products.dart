...
//  {this.ceva} = name parameter
//  [this.ceva] = optional param
//  (this.ceva) = required param
            ...
            children: <Widget>[
              FlatButton(
                ...
                onPressed: () => Navigator
                            //returns a <bool>
                            .pushNamed<bool>(
                                context, '/product/' + index.toString()) //4
                        .then((bool value) {
                      if (value) {
                        deleteProduct(index);
                      }
                    }),
//4 trebuie sa inceapa cu /, pt ca asta verifici
// in main.dart la onGenerateRoute (partea cu split('/)
                //apoi tre sa fie 'product' pt ca ai un if si pt asta mai jos tot in main.dart

//the push method returns a future(obj in waiting state, will eventually resolve,
//and you can listen to that moment when it resolves)
// asa ca putem sa chainuim stuff to it,
//ce vor fi rezolvate cand
//push() returns a generic type Future<T>
//ce returneaza push(), folosim ca argument pt then() - aici 'value'
